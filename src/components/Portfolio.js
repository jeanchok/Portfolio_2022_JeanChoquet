  ...
  return (
    <div className="main">
      ...
      <div className="portfolioContainer">
        ...
        <p className="tracking-in-expand-fwd-top">
          Découvrez mes projets réalisés en tant que développeur web.
        </p>
        <div className="cardContainer">
          <div className="cardContainer__side">
            <div>
              <h3 className="cardContainer__side--title">Sites Web</h3>
            </div>

            {projectsSite
              .filter((project) => project.putFirst === true)
              .map((projectFirst, index) => (
                <ProjectCardL key={projectFirst.id} project={projectFirst} />
              ))}
            {projectsSite
              .filter((project) => project.putFirst === false)
              .map((projectSmall, index) => (
                <ProjectCardS key={projectSmall.id} project={projectSmall} />
              ))}
          </div>
          <div className="cardContainer__side">
            <div>
              <h3 className="cardContainer__side--title">Applications Web</h3>
            </div>
            {projectsApp
              .filter((project) => project.putFirst === true)
              .map((projectFirst, index) => (
